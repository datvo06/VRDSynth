...
def batch_find_program_executor(nx_g, find_programs: List[FindProgram]) -> List[List[Tuple[Dict[WordVariable, str], Dict[RelationVariable, Tuple[WordVariable, WordVariable, int]]]]]:
    ...
    nx_g_removed_attrib = nx.DiGraph()
    for u in nx_g.nodes():
        nx_g_removed_attrib.add_node(u)
    for u, v, k, d in nx_g.edges(data=True, keys=True):
        nx_g_removed_attrib.add_edge(u, v)
    for path in path_to_programs:
        ...
        gm = isomorphism.DiGraphMatcher(nx_g_removed_attrib, nx_graph_query)
